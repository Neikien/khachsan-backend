...
async def create_service_usage(
    ...
    booking_result = await db.execute(
        select(Booking).where(Booking.MaDatPhong == service_usage.MaDatPhong)
    )
    ...
    service_result = await db.execute(
        select(Service).where(Service.MaDV == service_usage.MaDV)
    )
    ...
    if thanh_tien is None:
        ...
        thanh_tien = service.GiaDV * service_usage.SoLuong
...
async def get_service_usages_by_booking(
    ...
    result = await db.execute(
        select(ServiceUsage).where(ServiceUsage.MaDatPhong == booking_id)
    )
...
async def get_service_usage(
    ...
    result = await db.execute(
        select(ServiceUsage).where(ServiceUsage.MaSuDung == ma_su_dung)
    )
...
async def update_service_usage(
    ...
    if 'SoLuong' in update_data and 'ThanhTien' not in update_data:
        service_result = await db.execute(
            select(Service).where(Service.MaDV == item.MaDV)
        )
        service = service_result.scalar_one_or_none()
        if service:
            update_data['ThanhTien'] = service.GiaDV * update_data['SoLuong']
